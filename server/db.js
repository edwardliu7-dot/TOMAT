...
// Neon requires SSL even when a copied connection string omits
// `sslmode=require`. Keep the explicit flag for other hosted Postgres URLs,
// while preserving plain TCP for self-hosted Postgres instances.
const isNeonUrl = /(?:\.neon\.tech|\.neon\.dev)(?:[/:?]|$)/i.test(connectionString || '')
const wantsSsl = isNeonUrl || /sslmode=require|ssl=true/i.test(connectionString || '')
