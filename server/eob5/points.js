...
router.post('/', requireGuru, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO eob5_student_points (student_id, guru_id, jenis, poin, keterangan, tanggal)
       VALUES ($1,$2,$3,$4,$5,$6)
       RETURNING *`,
...
router.patch('/:id', requireGuru, async (req, res) => {
  try {
    ...
    const { rows: existing } = await pool.query(
      'SELECT student_id FROM eob5_student_points WHERE id = $1', [id]
    )
...
router.delete('/:id', requireGuru, async (req, res) => {
  try {
    ...
    await pool.query('DELETE FROM eob5_student_points WHERE id = $1', [id])
