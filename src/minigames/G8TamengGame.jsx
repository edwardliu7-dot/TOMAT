      ...
      <div style={{ padding: '0 16px 32px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <Card border="rgba(252,165,165,0.3)">
          <div style={{ textAlign: 'center', fontSize: 12, color: '#FCA5A5', fontWeight: 700, letterSpacing: 1, marginBottom: 12 }}>
            SUSUN FORMASI KSATRIA BERTAMENG
          </div>
          <div style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', marginBottom: 14 }}>
            Setiap formasi membentuk persegi simetris (Top-Down). Berapa jumlah tameng pada formasi ke-{q.start + 4}?
          </div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', justifyContent: 'center', flexWrap: 'wrap' }}>
            {q.terms.map((t, i) => (
              <div key={i} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 11, color: '#94A3B8', marginBottom: 4 }}>Formasi {q.start + i}</div>
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: '#94A3B8', marginBottom: 4 }}>Formasi {q.start + 4}</div>
        ...
        {feedback === null && (
          <Card>
            ...
            <div style={{ marginTop: 12 }}>
              <Btn onClick={confirm} disabled={digits === ''} color="#dc2626">Kunci Formasi</Btn>
            </div>
          </Card>
        )}
