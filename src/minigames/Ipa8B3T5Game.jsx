...
const ACCENT = '#ca8a04'
const BG = 'linear-gradient(180deg, #0f0a00 0%, #1e1400 100%)'
const TITLE = '🧫 Nephron Urine Factory'
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, gap: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexDirection: 'column', width: '100%', maxWidth: 320 }}>
        <button onClick={onRestart} style={{ padding: '14px', borderRadius: 14, border: 'none', background: ACCENT, color: '#1c1500', fontSize: 16, fontWeight: 800, cursor: 'pointer' }}>🔄 Main Lagi</button>
...
export default function Ipa8B3T5Game({ onBack }) {
      ...
      <div style={{ flex: 1, padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 600, margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {q.choices.map((c, i) => {
            ...
            if (selected !== null) {
              if (c === q.benar) { bg = '#ca8a0420'; border = '1.5px solid #ca8a04'; color = '#fde68a' }
