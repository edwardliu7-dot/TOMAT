...
export default function TaskResultScreen({ goBack, grade, navigateTo }) {
  ...
  const content = (
    <div style={{ padding: isMd ? 0 : '0 16px 40px' }}>
      {/* Hero card */}
      <div style={{
        marginTop: isMd ? 0 : 8, borderRadius: isMd ? 0 : 24, overflow: 'hidden',
        background: `linear-gradient(135deg, ${typeColor}18, ${typeColor}08)`,
        border: isMd ? 'none' : `1.5px solid ${typeColor}44`,
        borderBottom: isMd ? `1px solid ${typeColor}33` : undefined,
        padding: isMd ? '24px 0 20px' : 28, textAlign: 'center',
      }}>
      ...
      {/* Action buttons */}
      <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 10, paddingBottom: isMd ? 0 : 0 }}>
        <button
          onClick={() => navigateTo('grades')}
          style={{ background: typeColor, color: '#0A1628', border: 'none', borderRadius: 14, padding: '14px', fontSize: 15, fontWeight: 800, cursor: 'pointer', width: '100%', transition: 'opacity 0.15s' }}
        ...
        <button
          ...
          style={{ background: 'rgba(255,255,255,0.08)', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: '14px', fontSize: 14, fontWeight: 700, cursor: 'pointer', width: '100%', transition: 'opacity 0.15s' }}
  ...
  if (isMd) {
    return (
      <div style={{ minHeight: '100vh', background: '#0A0B14', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 24px' }}>
        <Confetti />
        <div style={{
          width: '100%', maxWidth: 480,
          borderRadius: 24, background: '#1A1D27',
