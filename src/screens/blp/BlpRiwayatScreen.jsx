...
export default function BlpRiwayatScreen({ goBack }) {
  ...
  const selectedSkor = selectedRecord ? hitungSkor(selectedRecord.completedActivities, false) : 0

  // Statistik keseluruhan
  const allSkors = sortedDates.map(d => hitungSkor(records[d].completedActivities, false))
                ...
                <div key={dateStr} style={{
                  ...
                  >
                    ...
                    <div style={{ fontSize: 11, color: '#6b7280', marginTop: 4 }}>
                      {rec.completedActivities?.length ?? 0} dari {AKTIVITAS_LIST.length} aktivitas • {isSelected ? '▲ Sembunyikan' : '▼ Lihat Detail'}
                    </div>
                  </button>
                        ...
                        {AKTIVITAS_LIST.map(a => {
                          ...
                          return (
                            <div key={a.id} style={{
                              ...
                              <span style={{ fontSize: 13 }}>{a.emoji} {a.label}</span>
                            </div>
                          )
                        })}
                      </div>
