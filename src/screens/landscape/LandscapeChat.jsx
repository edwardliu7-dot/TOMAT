...
export default function LandscapeChat({ goBack, initialTarget, initialTab: initialTabProp }) {
    ...
    return (
      <div key={m.id || i} style={{ display:'flex', justifyContent: mine ? 'flex-end' : 'flex-start' }}>
        <div style={{ maxWidth:'68%', background: mine ? 'linear-gradient(135deg,#3c3489,#2a2470)' : C.card, border: mine ? 'none' : `0.5px solid ${C.border}`, borderRadius: mine ? '12px 12px 3px 12px' : '12px 12px 12px 3px', padding: stick ? '4px 8px' : '7px 11px' }}>
          {!mine && <div style={{ color: tab === 'global' ? C.teal : C.green, fontSize:8, fontWeight:700, marginBottom:2 }}>{m.sender_name || m.name || '—'}</div>}
          {stick ? (
  ...
  const renderInput = (disabled = false) => (
    <div style={{ padding:'7px 12px', borderTop:`0.5px solid #1e2644`, display:'flex', gap:7, alignItems:'center', flexShrink:0 }}>
      ...
      />
      <div onClick={(sending || disabled) ? null : sendMessage} style={{ width:34, height:34, borderRadius:'50%', background:`linear-gradient(135deg,${C.orange},#c94f2d)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:14, cursor: (sending || disabled) ? 'not-allowed' : 'pointer', opacity: (sending || disabled) ? 0.5 : 1 }}>➤</div>
    </div>
  )
