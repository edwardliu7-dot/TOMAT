...
const GRADE_DATA = {
  7: {
    babs: [
      { label:'BAB I\nBilangan Bulat', games:7, icon:'🔢' },
      { label:'BAB II\nHimpunan', games:6, icon:'⭕' },
      { label:'BAB III\nAljabar', games:7, icon:'📐' },
    ]
  },
  8: {
    babs: [
      { label:'BAB I\nPola Bilangan', games:6, icon:'🔢' },
      { label:'BAB II\nPythagoras', games:6, icon:'📐' },
      { label:'BAB III\nGeometri', games:6, icon:'🔺' },
      { label:'BAB IV\nBangun Datar', games:8, icon:'◼' },
      { label:'BAB V\nStatistika', games:7, icon:'📊' },
      { label:'BAB VI\nPerluasan', games:5, icon:'➕' },
      { label:'BAB VII\nPerluasan', games:5, icon:'✖' },
    ]
  },
  9: {
    babs: [
      { label:'BAB I\nPerpangkatan', games:6, icon:'🔢' },
      { label:'BAB II\nPersamaan', games:7, icon:'⚖' },
      { label:'BAB III\nBangun Ruang', games:6, icon:'📦' },
      { label:'BAB IV\nTransformasi', games:7, icon:'🔄' },
      { label:'BAB V\nStatistika', games:5, icon:'📊' },
    ]
  }
}
    ...
    <div style={{ width:'100vw', height:'100vh', background:C.bg, fontFamily:'system-ui,sans-serif', display:'flex', flexDirection:'column', overflow:'hidden', position:'relative' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 16px 8px', borderBottom:'0.5px solid #1e2644', flexShrink:0, position:'relative', zIndex:2 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <span style={{ color:C.txt, fontSize:15, fontWeight:700 }}>Zona Matematika</span>
        </div>
      </div>
      ...
      <div style={{ flex:1, display:'flex', minHeight:0, position:'relative', zIndex:2 }}>
        ...
        <div style={{ width:'22%', borderRight:'0.5px solid #1e2644', display:'flex', flexDirection:'column', padding:'10px 8px', gap:7 }}>
          {[7,8,9].map(g => (
            <div key={g} onClick={() => setActiveGrade(g)} style={{ background: g===activeGrade?C.purpleBg:C.card, border: g===activeGrade?'none':`0.5px solid ${C.border}`, borderRadius:10, padding:'10px', flex: g===activeGrade?1.5:1, display:'flex', flexDirection:'column', justifyContent:'center', cursor:'pointer', transition:'all 0.15s' }}>
              <div style={{ color: g===activeGrade?'#eeedfe':C.sub, fontSize:12, fontWeight: g===activeGrade?700:500 }}>Kelas {g}</div>
              <div style={{ color: g===activeGrade?C.purple:C.muted, fontSize:8.5, marginTop:3 }}>{GRADE_DATA[g].babs.length} BAB</div>
              <div style={{ marginTop:6, height:2.5, background: g===activeGrade?'rgba(255,255,255,0.15)':'#2a3158', borderRadius:2 }}>
                <div style={{ height:2.5, background: g===activeGrade?C.purple:C.purpleBg, borderRadius:2, width: g===activeGrade?'38%':'0%' }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ flex:1, padding:'10px 14px', display:'flex', flexDirection:'column', gap:7 }}>
          ...
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', overflowY:'auto' }}>
            {babs.map((bab, i) => (
              <div key={i} onClick={() => navigate(GRADE_ROUTES[activeGrade])} style={{ background:C.card, border:`0.5px solid ${C.border}`, borderRadius:11, padding:'11px 12px', width:140, flexShrink:0, cursor:'pointer', transition:'all 0.15s', position:'relative' }}>
                ...
                <div style={{ marginTop:6, height:3, background:'#2a3158', borderRadius:2 }}>
                  <div style={{ height:3, background:C.purple, borderRadius:2, width:'0%' }} />
                </div>
                <div style={{ position:'absolute', top:8, right:8 }}>
                  <div style={{ background:C.purpleBg, borderRadius:5, padding:'2px 6px', color:'#eeedfe', fontSize:8, fontWeight:700 }}>Masuk ▶</div>
                </div>
              </div>
            ))}
          </div>
