// Zona IPA — Peta BAB, Landscape 844×390
export function ZonaIPA() {
  const grades = [
    { label:"Kelas 7", babs:["BAB I\nPengukuran", "BAB II\nZat & Wujud", "BAB III\nSuhu & Kalor", "BAB IV\nEnergi & Gerak"], done:[4,2,0,0], total:[5,5,6,5], color:"#085041" },
    { label:"Kelas 8", babs:["BAB I\nGaya & Gerak", "BAB II\nSel & Organ", "BAB III\nSistem Tubuh"], done:[3,1,0], total:[7,6,7], color:"#085041" },
    { label:"Kelas 9", babs:["BAB I\nReproduksi", "BAB II\nListrik", "BAB III\nKelangsungan Hidup"], done:[2,0,0], total:[4,6,5], color:"#085041" },
  ];
  const activeGrade = 0;
    ...
    <div style={{ width:844, height:390, background:"#12172b", fontFamily:"system-ui,sans-serif", display:"flex", flexDirection:"column", overflow:"hidden", position:"relative" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 14px 8px", borderBottom:"0.5px solid #1e2644", flexShrink:0, position:"relative", zIndex:2 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <span style={{ color:"#f2ede3", fontSize:14, fontWeight:700 }}>Zona IPA</span>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:5 }}>
          ...
          <div style={{ background:"#1c2340", border:"0.5px solid #313a5c", borderRadius:7, padding:"3px 9px" }}>
            <span style={{ color:"#5dcaa5", fontSize:11, fontWeight:800 }}>30%</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex:1, display:"flex", minHeight:0, position:"relative", zIndex:2 }}>
        ...
        <div style={{ width:130, borderRight:"0.5px solid #1e2644", display:"flex", flexDirection:"column", padding:"10px 8px", gap:6 }}>
          {grades.map((gr,i) => (
            <div key={i} style={{ background: i===activeGrade?"#085041":"#1c2340", border: i===activeGrade?"none":"0.5px solid #313a5c", borderRadius:9, padding:"9px 10px", cursor:"pointer", flex: i===activeGrade?1.5:1, display:"flex", flexDirection:"column", justifyContent:"center" }}>
              <div style={{ color: i===activeGrade?"#e1f5ee":"#8b8f9e", fontSize:11, fontWeight: i===activeGrade?700:500 }}>{gr.label}</div>
              <div style={{ marginTop:5, height:2.5, background: i===activeGrade?"rgba(255,255,255,0.15)":"#2a3158", borderRadius:2 }}>
                <div style={{ height:2.5, background: i===activeGrade?"#9fe1cb":"#085041", borderRadius:2, width:`${Math.round(gr.done.reduce((a,b)=>a+b,0)/gr.total.reduce((a,b)=>a+b,0)*100)}%` }} />
              </div>
              <div style={{ color: i===activeGrade?"#9fe1cb":"#5a6180", fontSize:8, marginTop:2 }}>
        ...
        <div style={{ flex:1, padding:"10px 12px", display:"flex", flexDirection:"column", gap:6 }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {g.babs.map((bab,i) => {
              ...
              const locked = i>2;
              return (
                <div key={i} style={{ background: locked?"#161c33": pct===100?"linear-gradient(135deg,#0d2a20,#1a3a2a)":"#1c2340", border: locked?"0.5px dashed #313a5c": pct===100?"0.5px solid #2a5040":"0.5px solid #313a5c", borderRadius:10, padding:"10px", width:150, flexShrink:0, opacity: locked?0.5:1, cursor: locked?"default":"pointer", position:"relative" }}>
                  ...
                  {!locked && (
                    <div style={{ marginTop:5, height:2.5, background:"#2a3158", borderRadius:2 }}>
                      <div style={{ height:2.5, background: pct===100?"#5dcaa5":"#9fe1cb", borderRadius:2, width:`${pct}%` }} />
                    </div>
                  )}
                </div>
              );
