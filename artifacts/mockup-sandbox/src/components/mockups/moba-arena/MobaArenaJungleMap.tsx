...
type Node = { id: string; x: number; y: number; label: string; points: number; tone: "mint" | "gold" | "coral" };
type Pet = { name: string; letter: string; x: number; y: number; color: string; team: "pijar" | "rona"; self?: boolean; carrying?: boolean };

const nodes: Node[] = [
  { id: "keliling", x: 24, y: 25, label: "Keliling", points: 10, tone: "mint" },
  { id: "pecahan", x: 43, y: 62, label: "Pecahan", points: 25, tone: "gold" },
  { id: "peluang", x: 62, y: 24, label: "Peluang", points: 50, tone: "coral" },
  { id: "luas", x: 75, y: 68, label: "Luas", points: 10, tone: "mint" },
  { id: "pola", x: 80, y: 32, label: "Pola", points: 25, tone: "gold" },
  { id: "sudut", x: 53, y: 83, label: "Sudut", points: 10, tone: "mint" },
];
...
function Pet({ pet }: { pet: Pet }) {
  return (
    <div className={`j-pet ${pet.self ? "j-self" : ""}`} style={{ left: `${pet.x}%`, top: `${pet.y}%`, "--pet": pet.color } as CSSProperties}>
      ...
      <div className="j-body"><i className="j-ear e-left" /><i className="j-ear e-right" /><b>{pet.letter}</b><i className="j-eye e-left" /><i className="j-eye e-right" /></div>
    ...
    <main className="j-arena">
      <style>{`
        ...
        *{box-sizing:border-box}html,body,#root{margin:0;min-width:100%;min-height:100%;overflow:hidden}button{font:inherit}
        ...
        .j-board{position:absolute;inset:4% 8%;border:1px solid rgba(203,227,164,.22);transform:perspective(1000px) rotateX(5deg) rotateZ(-1deg);box-shadow:0 0 0 24px rgba(4,32,39,.13),0 30px 90px rgba(0,10,16,.48);overflow:hidden;border-radius:2.2%}
        ...
        .j-brush{position:absolute;z-index:2;width:clamp(74px,9vw,145px);height:clamp(48px,5vw,82px);border-radius:55% 45% 48% 42%;background:radial-gradient(circle at 27% 30%,#9fc66f 0 10%,transparent 11%),radial-gradient(circle at 66% 21%,#75ab68 0 15%,transparent 16%),radial-gradient(circle at 40% 73%,#4f9565 0 23%,transparent 24%),radial-gradient(circle at 77% 67%,#87b66a 0 22%,transparent 23%),#36785d;filter:drop-shadow(0 10px 4px rgba(2,34,36,.42));animation:j-sway 4s ease-in-out infinite}
        ...
        .j-rock{position:absolute;z-index:3;width:clamp(38px,4.2vw,74px);height:clamp(25px,3vw,52px);border-radius:43% 54% 34% 48%;background:linear-gradient(145deg,#8eafb0,#365b64 72%);border:2px solid rgba(202,223,188,.32);box-shadow:0 8px 0 rgba(3,35,39,.42);transform:rotate(-10deg)}.r1{left:28%;top:34%}.r2{left:32%;top:39%;transform:rotate(18deg) scale(.65)}.r3{right:26%;top:33%;transform:rotate(13deg)}.r4{right:22%;top:39%;transform:scale(.7)}.r5{left:56%;bottom:21%;transform:rotate(22deg) scale(.8)}
        ...
        .j-node{position:absolute;z-index:8;width:53px;height:53px;display:grid;place-items:center;border-radius:50%;transform:translate(-50%,-50%);color:#fff5cc;border:3px solid #e8e5a8;cursor:pointer;box-shadow:0 8px 12px rgba(3,31,34,.34);animation:j-pulse 2.4s ease-in-out infinite}.j-node.mint{background:radial-gradient(circle at 35% 25%,#d5eca1,#3a956f)}.j-node.gold{background:radial-gradient(circle at 35% 25%,#ffe9ad,#bd7a3f)}.j-node.coral{background:radial-gradient(circle at 35% 25%,#ffd0af,#bb544e)}.j-node.claimed{opacity:.32;filter:grayscale(1);animation:none}.j-node:hover{transform:translate(-50%,-50%) scale(1.15)}.j-node span{position:absolute;top:58px;white-space:nowrap;font:700 8px 'Space Mono';text-shadow:0 2px 4px #0a3439}.j-node b{position:absolute;right:-19px;top:3px;font:700 9px 'Space Mono'}
        .j-pet{position:absolute;z-index:10;width:72px;text-align:center;transform:translate(-50%,-50%);animation:j-float 3s ease-in-out infinite}.j-pet:nth-of-type(2n){animation-delay:1.1s}.j-name{position:relative;z-index:3;display:inline-flex;gap:4px;padding:3px 6px;color:#fff5cd;background:rgba(7,40,44,.78);border:1px solid rgba(243,225,158,.42);border-radius:5px;font-size:9px;font-weight:700;white-space:nowrap}.j-name em{color:#ffc08c;font-style:normal;font-size:8px}.j-self .j-name{border-color:#ffbd8d;background:rgba(109,53,49,.84)}.j-shadow{position:absolute;z-index:1;bottom:0;left:13px;width:48px;height:12px;background:rgba(1,28,31,.42);filter:blur(3px);border-radius:50%}.j-body{position:relative;z-index:2;width:43px;height:38px;margin:7px auto 0;display:grid;place-items:center;color:#fff2bd;background:linear-gradient(145deg,color-mix(in srgb,var(--pet) 75%,white),var(--pet));border:3px solid #ffe8ad;border-radius:48%;font:700 15px 'Space Mono';box-shadow:0 5px 0 rgba(3,34,38,.38);animation:j-walk .6s ease-in-out infinite alternate}.j-ear{position:absolute;top:-7px;width:14px;height:14px;border:2px solid #ffe8ad;background:var(--pet);border-radius:5px 9px 4px 8px}.j-ear.e-left{left:1px;transform:rotate(-25deg)}.j-ear.e-right{right:1px;transform:rotate(25deg)}.j-eye{position:absolute;top:14px;width:5px;height:7px;border-radius:50%;background:#163c42}.j-eye.e-left{left:9px}.j-eye.e-right{right:9px}.j-scroll{position:absolute;z-index:4;right:2px;top:29px;width:21px;height:21px;display:grid;place-items:center;border:2px solid #ffefb5;border-radius:50%;color:#a66a35;background:#ffe09b;transform:rotate(10deg)}.j-team{position:absolute;z-index:4;left:8px;top:29px;width:7px;height:7px;border:1px solid #fff3c5;border-radius:50%}.j-team.pijar{background:#ee735d}.j-team.rona{background:#7c8de1}
        ...
        .j-map-pop{position:absolute;z-index:40;top:162px;right:28px;width:275px;padding:11px;border:1px solid rgba(239,222,155,.36);border-radius:14px;background:rgba(6,39,44,.93);box-shadow:0 18px 36px rgba(1,21,27,.48);backdrop-filter:blur(10px)}.j-map-pop header{display:flex;justify-content:space-between;align-items:center;margin-bottom:9px;font-size:11px;font-weight:700}.j-map-pop header span{display:flex;gap:6px;align-items:center}.j-map-pop header button{display:grid;place-items:center;border:0;background:none;color:#d6d3a5;cursor:pointer}.j-map-large{position:relative;height:155px;overflow:hidden;border-radius:9px;background:linear-gradient(135deg,#266b61,#123e4c)}.j-map-large:before{content:"";position:absolute;inset:0;background-image:linear-gradient(rgba(190,214,145,.14) 1px,transparent 1px),linear-gradient(90deg,rgba(190,214,145,.14) 1px,transparent 1px);background-size:20px 20px}.mm-river{position:absolute;inset:-20% 41%;transform:rotate(16deg);background:#38969b;clip-path:polygon(30% 0,72% 0,100% 100%,0 100%)}.mm-base{position:absolute;z-index:2;width:19px;height:19px;border-radius:50%;border:2px solid #ffecb4}.mm-pijar{left:9%;bottom:10%;background:#db705c}.mm-rona{right:9%;top:10%;background:#7386dc}.mm-node{position:absolute;z-index:3;width:7px;height:7px;border-radius:50%;background:#e5ba5a;border:1px solid #fff0bf}.mm-player{position:absolute;z-index:4;left:26%;top:55%;width:9px;height:9px;border-radius:50%;background:#f0785d;border:2px solid #fff3c4}.j-map-pop small{display:block;margin-top:8px;color:#b5c998;font-size:9px}
      ...
      <section className="j-world" aria-label="Peta hutan TOMAT">
        <div className="j-board"><div className="j-grid" /><div className="j-lane a" /><div className="j-lane b" /><div className="j-lane c" /><span className="j-lane-label jl-a">LAJUR UTARA</span><span className="j-lane-label jl-b">LAJUR TENGAH</span><span className="j-lane-label jl-c">LAJUR SELATAN</span>
          ...
          <span className="j-rock r1" /><span className="j-rock r2" /><span className="j-rock r3" /><span className="j-rock r4" /><span className="j-rock r5" />
          ...
          {nodes.map((node) => <button key={node.id} className={`j-node ${node.tone} ${claimed.includes(node.id) ? "claimed" : ""}`} style={{ left: `${node.x}%`, top: `${node.y}%` }} onClick={() => openNode(node)} aria-label={`${node.label}, ${node.points} poin`}><CircleHelp size={18} /><b>+{node.points}</b><span>{node.label}</span></button>)}
